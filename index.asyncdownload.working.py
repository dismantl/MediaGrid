...
class PubDir(resourcelib.Resource):
	...
	self.request.setHeader('ETag', (response.headers.getRawHeaders('ETag'),)[0][0])
	self.request.setHeader('Content-Type', (response.headers.getRawHeaders('Content-Type'),)[0][0])
	self.request.setHeader('Content-Disposition', 'attachment; filename=%s;' % str(self.request)[:len(str(self.request))-10].rpartition('/')[2])
	finished = Deferred()
	...
	#d = getPage('http://www.google.com', method='GET', postdata=None, headers={'user-agent': 'Mozilla/5.0 (X11; Linux x86_64; rv:12.0) Gecko/20100101 Firefox/12.0'})
